...
int main(int argc,char** argv)
{
    ...
    if(argc==3&&reg_info_file==NULL)
    {
       ...
       if(str_to_hex32_bits(argv[2],&val)==-1)
       {
          fprintf(stderr,"wrong value format %s\n",argv[2]);
          exit(3);
       }
    }
...
//DEBUG("FPGA_ID=0x%08X in phy/vir addr 0x%08X/%p\n",*(uint32_t*)(addr+FPGA_ID),PROTO_BASE_ADDRESS+FPGA_ID,addr+FPGA_ID);
    ...
    if(reg_info_file!=NULL)
       multi_mem_write_from_file(reg_info_file,addr,0,PROTO_SIZE);
    else if(argc==3)
       mem_write(addr,0,mem_addr,val);
    else
       mem_write_from_file(stdin,addr,0,PROTO_SIZE);
